...
// Byte swapping function (only needed on some systems)
static inline uint32_t swap_argb_bytes(uint32_t argb) {
  return ((argb & 0xff000000) >> 0) |  // Keep alpha in the same place
    ((argb & 0x00ff0000) >> 16) | // Move red to blue
    ((argb & 0x0000ff00) >> 0) |  // Keep green in the same place
...
int main() {
  ...
  for (int y = 0; y < imageHeight; y++) {
    for (int x = 0; x < imageWidth; x++) {
      ...
      // Swap bytes if necessary (system/libcaca dependent)
      pixel = swap_argb_bytes(pixel);
